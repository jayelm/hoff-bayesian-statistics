...
$$\bar{Y} \mid \sigma^2, y_1, \dots, y_n \sim \mathcal{N}(\mu_n, \tau_n^2 + \sigma^2)$$
...
\begin{align}
p(y \mid \boldsymbol{\phi}) &= \frac{1}{\sqrt{\pi}} | \phi_2 |^{1/2} \text{exp}\left( \frac{\phi_1^2}{2\phi_2} \right) \text{exp} \left( \begin{pmatrix} \theta/\sigma^2 \\ -(2\sigma^2)^{-1} \end{pmatrix} \begin{pmatrix} y & y^2 \end{pmatrix} \right) \\
&= \frac{1}{\sqrt{\pi}} (2\sigma^2)^{-1/2} \text{exp}\left( \frac{(\theta / \sigma^2)^2}{-2(2\sigma^2)^{-1}} \right) \text{exp} \left( \begin{pmatrix} \theta/\sigma^2 \\ -(2\sigma^2)^{-1} \end{pmatrix} \begin{pmatrix} y & y^2 \end{pmatrix} \right) \\
\end{align}
...
<!--
Yes, but because I know, after some poking around, this is a
[normal-inverse-chi-squared 
function](https://www2.stat.duke.edu/courses/Fall10/sta114/notes15.pdf). For random variables $W, V$, the density function is
