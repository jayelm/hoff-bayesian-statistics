...
Since Gamma distributions have a well-defined PDF this integral is computable,
but alternatively, we can estimate via Monte Carlo estimation.
...
algorithmically sampling from a Normal distribution, see the [Box-Muller 
transform](https://en.wikipedia.org/wiki/Box%E2%80%93Muller_transform). However,
...
$\frac{1}{S} \sum \theta^i = \mathbb{E}(p(\theta))$. This also implies that the
...
this, we bootstrap the number of samples we ought to take to report our estimate
...
$\theta_1$ and $\theta_2$ are independent, we can sample a pair $(\theta^i_1,
\theta^i_2)$ from separate samples from $p(\theta_1 \mid y)$ and $p(\theta_2
\mid y)$, such that many such pairs $(\theta^i_1, \theta^i_2)$ will (by Monte
...
sample multiple pairs $(\theta^i_1, \theta^i_2)$, and multiple pairs $\gamma^i =
f(\theta^i_1, \theta^i_2)$.
...
Note: there are probably some theoretical results about variance of the samples
from this kind of sampling that means that we would need to sample more $y_i$s
than $\theta_i$s since we're doing "double" sampling. (Probably use the same
techniques of estimating standard error)
...
Appears to be a roughly linear relationship. Even a strong factor of $n_0$ has a
...
\sum y_i, b)$. (The posterior predictive parameters are the same as those of the
...
See https://www2.stat.duke.edu/courses/Fall11/sta114/jeffreys.pdf for some
insight on this.
