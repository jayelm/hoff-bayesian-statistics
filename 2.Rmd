...
$\mathcal{Y}$ is *countable*. Examples include the binomial and poisson 
distributions.

$Y$ is *continuous* if $\mathcal{Y}$ is *uncountable*. Note that the probability
of $Y$ taking a single value in the sample space is 0. So we use cumulative
distribution functions (CDFs) to define probability distributions
...
$$
F(y_1, y_2) = \int_{-\infty}^{x} \int_{-\infty}^{y} P(y_1, y_2) \; dy_2 dy_1.
$$
...
We say that $Y_1, \dots, Y_n$ are conditional independent and identically distributed (i.i.d.):
...
> de Finetti's theorem. Let $Y_1, \dots, Y_n$ be exchangable but **not necessarily i.i.d.** random variables: $$P(y_1, \dots, y_n) = P(y_{\pi_1}, \dots, y_{\pi_n})$$ for all partitions $\pi$. Then our model can be written as
...
Importantly, if we sample from a sufficiently large population, then we can model
the sample variables as being approximately conditionally i.i.d.
...
values in $\{0, 1\}$, there is a probability distribution $F$ such that
...
Also denote $(x)_k = \prod_{j = 0}^{k - 1} (x - j)$, that is, the first $k$
numbers of the computation of the factorial starting at $x, x-1, \dots, 1$: e.g.
$(7)_2 = 7 * 6 = 42$.
...
a sequence of length $m$. We can (somehow) re-package the summation of discrete
$r$ as an integral over a stepwise function $F_m$ that jumps at every $r$ from $0$ to $m$.


If you imagine what happens to the integrand as $m$ goes to infinity:

\begin{align}
\lim_{m \to \infty} \frac{5}{2}
\end{align}
