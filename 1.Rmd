...
ggplot(df, aes(x = w, y = theta0, fill = theta)) +
  geom_tile() +
...
proportion, but falls short here. Although the Wald interval is asymptotically
correct in its level of confidence, for small sample sizes, this property does
not hold. For an observation $Y = 20$, for example, the Wald CI is, regardless
of level of confidence, just $0$. We wouldn't want to say with 99.999%
confidence that the population mean is 0, given our small sample size.
...
Notice that as $n$ grows large, this estimator approaches the sample mean $\frac{y}{n}$:

$$
\lim_{n \to \infty} \frac{n}{n + w} \frac{y}{n} + \frac{w}{n + w}\theta_0 =
\frac{y}{n}.
$$

This captures the intuition that with an infinite amount of data, our priors, 
regardless of estimate and confidence, are "overwhelmed" by the data.
...
A brief synonpsis of an example in Chapter 9, where we want to build a 
