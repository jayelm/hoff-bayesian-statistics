...
\begin{align}
p(\boldsymbol{\theta} \mid \mathbf{y}) &= \frac{\mathbf{y} \mid
p(\boldsymbol{\theta})p(\boldsymbol{\theta})}{\int p(\mathbf{y} \mid
...
$\theta_A$ and $\theta_B$ are dependent, since $p(\theta_B \mid \theta_A = 10) =
10\gamma) \neq 5\gamma = p(\theta_B \mid \theta_A = 5)$. This prior is justified
if we have reason to believe that $\theta_B$ is some product of $\theta_A$ plus
random Gamma-distributed noise.

### b

First the full conditional

\begin{align}
p(\theta, \gamma \mid \mathbf{y}_A, \mathbf{y}_B)
&\propto p(\theta, \gamma) \times p(\mathbf{y}_A, \mathbf{y}_B \mid \theta, \gamma) \\
&\propto p(\theta) \times p(\gamma) \times p(\mathbf{y}_A \mid \theta) \times p(\mathbf{y}_B \mid \theta, \gamma) & \text{$\mathbf{y}_A \perp \gamma$} \\
&\propto \left(\theta^{a_\theta - 1}e^{-b_\theta \theta}\right) \times \left(\gamma^{a_\gamma - 1}e^{-b_\gamma \gamma} \right) \times  \left(\prod_{i=1}^{n_{A}} \theta^{y_{A_i}} e^{-\theta} \right) \times \left(\prod_{i=1}^{n_{B}} \left( \text{Product of two Gammas??} \right) \right) \\
\end{align}


```{r}
hist(rgamma(5000, 2, 3) * rgamma(5000, 4, 5))
df = data.frame(
  gamma = seq(0, 6, length = 500),
  density = dgamma(seq(0, 6, length = 500), 2 * 4, 3 * 5)
)
qplot(x = df$gamma, y = df$density, geom = 'line')
```